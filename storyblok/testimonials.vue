<template>
  <section v-editable="blok" class="testimonials">
    <div class="relative bg-brand-yellow pt-16 pb-24 md:pt-24 md:pb-32 lg:pt-28 lg:pb-36">
      ...
      <button @click="testimonyCarousel.prev()" class="hidden md:block absolute z-10 left-12 xl:left-20 bottom-1/2 translate-y-1/2 w-8 h-8 md:w-12 md:h-12 rounded-full cursor-pointer">
        <svgo-arrow-left filled class="text-brand-red" />
      </button>
      <button @click="testimonyCarousel.next()" class="hidden md:block absolute z-10 right-12 xl:right-20 bottom-1/2 translate-y-1/2 w-8 h-8 md:w-12 md:h-12 rounded-full cursor-pointerr">
